...
namespace { // private module-only namespace
    ...
    public:
        ...
        readyStateValues getReadyState() const { return readyStateValues::CLOSED; }
    ...
    public:
        ...
        _RealWebSocket(socket_t sockfd, bool useMask) : sockfd(sockfd), readyState(readyStateValues::OPEN),
                                                        useMask(useMask) {
        }
        ...
        void poll(int timeout) { // timeout in milliseconds
            if (readyState == readyStateValues::CLOSED) {
                if (timeout > 0) {
            ...
            while (true) {
                ...
                if (false) {}
                    ...
                    rxbuf.resize(N);
                    closesocket(sockfd);
                    readyState = readyStateValues::CLOSED;
            ...
            if (!txbuf.size() && readyState == readyStateValues::CLOSING) {
                closesocket(sockfd);
                readyState = readyStateValues::CLOSED;
            }
        }
        ...
        void
        sendData(wsheader_type::opcode_type type, uint64_t message_size, Iterator message_begin, Iterator message_end) {
            // TODO:
            ...
            if (readyState == readyStateValues::CLOSING || readyState == readyStateValues::CLOSED) { return; }
    ...
    easywsclient::WebSocket::pointer from_url(const char *url, bool useMask) {
        char host[128];
        int port;
        char path[128];
        if (sizeof(url) >= 128) {
        ...
        }
        {
            ...
            ::send(sockfd, line, strlen(line), 0);
            snprintf(line, 256, "Sec-WebSocket-protocol: mrtc-protocol\r\n");
            ::send(sockfd, line, strlen(line), 0);
            snprintf(line, 256, "\r\n");
            ::send(sockfd, line, strlen(line), 0);
...
namespace easywsclient {
    ...
    WebSocket::pointer WebSocket::from_url(const char *url) {
